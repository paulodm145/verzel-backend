...
- [x] **T-1** — `qrcode.service.ts` e as variáveis `TICKET_SECRET` e
      `APP_BASE_URL`.
...
- [x] **T-2** — Emissão do ingresso na transação do pagamento aprovado.
...
- [x] **T-3** — Repositório, service e rotas de ingresso.
...
- [x] **T-4** — Documentar ingressos no OpenAPI.
...
- [x] Checkbox do Épico 5 no backlog, e status no roadmap
- [x] `TICKET_SECRET` no `.env.example`, sem valor real
