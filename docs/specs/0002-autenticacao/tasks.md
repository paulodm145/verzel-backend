...
- [ ] **T-1** — `password.service.ts`: derivar e verificar hash com scrypt,
...
- [ ] **T-2** — Variáveis novas no `env.ts` e no `.env.example`: `JWT_SECRET`
...
- [ ] **T-3** — Instalar `jose` (`^6.2.8`, única dependência nova do épico) e
...
- [ ] **T-4** — Modelo `RefreshToken` no schema Prisma e migration aditiva.
  - Commit: `feat(prisma): modelar o token de renovação`
  - Cobre: RN-5, RN-6
  - Testes: integração — `tokenHash` duplicado é rejeitado; apagar o usuário
    apaga suas sessões

- [ ] **T-5** — `auth.schema.ts` com os schemas de entrada e saída do plan.
...
- [ ] **T-6** — `auth.repository.ts`: interface `AuthRepository` e implementação
...
- [ ] **T-7** — `auth.service.ts`, cadastro e login: papel fixo em `CUSTOMER`,
...
- [ ] **T-8** — `auth.service.ts`, rotação e revogação: renovar troca o token em
...
- [ ] **T-9** — `authenticate.ts` e `require-role.ts`, com o contexto no
      `WeakMap` e `getAuth`.
...
- [ ] **T-10** — `auth.controller.ts`, `auth.routes.ts` e montagem no `app.ts`.
  - Commit: `feat(auth): expor as rotas de sessão`
  - Cobre: CA-1, CA-5, CA-7
  - Testes: integração — fluxo `register → login → me → refresh → logout`

- [ ] **T-11** — Documentar as rotas no OpenAPI, com o esquema de segurança
...
- [ ] **T-12** — `prisma/seed.ts` idempotente com um organizador, dois clientes e
...
- [ ] **T-13** — README: variáveis novas, credenciais de teste, como rodar o seed
...
- [ ] Os quinze critérios cobertos por teste automatizado
- [ ] `npm run lint` e `npm run typecheck` sem erro
- [ ] `npm test` verde
- [ ] As cinco rotas visíveis em `/docs`, com o esquema Bearer
- [ ] Credenciais de teste no README, e todas autenticam
- [ ] Checkbox do Épico 2 marcado no backlog do `CLAUDE.md`
- [ ] Status da spec 0002 atualizado no roadmap, junto com o da 0001, que ficou
