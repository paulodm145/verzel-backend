...
- [ ] **T-1** — `shared/lib/lock.ts` e as variáveis de prazo no ambiente.
...
- [ ] **T-2** — `shared/middlewares/idempotency.ts`.
...
- [ ] **T-3** — Schemas e repositório de reserva, traduzindo violação de
...
- [ ] **T-4** — `reservations.service.ts`: lock, expiração preguiçosa, dono,
      cancelamento.
...
- [ ] **T-5** — Rotas de reserva e montagem.
  - Commit: `feat(reservations): expor as rotas de reserva`
  - Cobre: CA-1, CA-13, CA-14
  - Testes: integração

- [ ] **T-6** — `payments.service.ts` e rota de pagamento.
...
- [ ] **T-7** — Testes de concorrência contra Postgres real, com e sem Redis.
...
- [ ] **T-8** — Documentar reserva e pagamento no OpenAPI, incluindo o cabeçalho
      de idempotência.
...
- [ ] O teste de concorrência passa com o Redis derrubado
- [ ] `npm run lint`, `npm run typecheck` e `npm test` sem erro
- [ ] Rotas novas no `/docs`
- [ ] Checkbox do Épico 4 no backlog, e status no roadmap
