...
- [x] **T-1** — Contrato do catálogo (`catalog.types.ts`, `catalog.port.ts`) e as
...
- [x] **T-2** — Adapters do TMDb e do Ticketmaster, com `fetch` injetado, prazo
...
- [x] **T-3** — `catalog.factory.ts`: instancia só o que tem chave.
  - Commit: `feat(catalog): escolher os provedores por configuração`
  - Cobre: RN-2
  - Testes: unitário — nenhuma chave, uma chave, as duas

- [x] **T-4** — `catalog.service.ts`: agrega com `allSettled` e cacheia no Redis.
...
- [x] **T-5** — `catalog.schema.ts` e `GET /catalog/search` restrita a organizador.
...
- [x] **T-6** — `events.schema.ts` com criação, edição, filtro e saída.
  - Commit: `feat(events): definir os schemas de evento`
  - Cobre: —
  - Testes: unitário dos limites (capacidade, preço, paginação)

- [x] **T-7** — `events.repository.ts`, incluindo criação de evento com mapa de
...
- [x] **T-8** — `events.service.ts`: dono, transições de estado e regeneração do
      mapa em rascunho.
...
- [x] Os catorze critérios cobertos por teste automatizado
- [x] `npm run lint`, `npm run typecheck` e `npm test` sem erro
- [x] Rotas novas no `/docs`
- [x] Checkbox do Épico 3 no backlog do `CLAUDE.md`, e status no roadmap
- [x] Nenhum segredo commitado; `.env.example` com as chaves novas
